...
function $emit(key, args, pathname) {
    var event = this.$events
    if (event && event[key]) {
        if (args) {
            args[2] = pathname || key   //pathname是确切的参数值 by13
        }
        var arr = event[key]
        ...
        if (parent) {
            ...
            $emit.call(parent, "*." + key, args, this.$pathname)//以模糊的值往上冒泡	//by13 以确认值上传
        }
    } else {
        parent = this.$up
        if (this.$ups) {
        ...
        if (parent) {
            ...
            if (arr.indexOf("*") === -1) {
                ...
                arr[1] = "*"
                $emit.call(parent, arr.join("."), args, path)//以模糊的值往上冒泡 //by13 以确认值上传
            } else {
                $emit.call(parent, path, args)//以确切的值往上冒泡
            }
        }
    ...
    if (xtype === "array") {
        ...
        for (i in val) {
            ...
            if (val.hasOwnProperty(i)) {
                var value = val[i]
				if (i.charAt(0) === '$') {  //排除以$开头的property，节省性能，暂未发现因为不完全拷贝导致可能发生的数据突变 by13
                    obj[i] = value;
                } else {
...
var newProto = {
    ...
    update: function (index, val) {
        ...
        if (((index >>> 0) === index) && this[index] !== val) {
            if (index > this.length) {
                throw Error(index + "update方法的第一个参数不能大于原数组长度")
            }
            if (typeof this[index] !== "undefined") {
                if (avalon.type(val) === "object") {
                    //合并原始值
                    //val = avalon.mix({}, this[index] ? this[index].$model : {}, val)
                    $emit.call(this.$up, this.$pathname + ".*", [val, this[index]])
                    for(k in val) {
...
function scanNodeArray(nodes, vmodels) {
    function _delay_component(name) {
        ...
        }, 90)	//给予较长一个延时等待, 否则会产生组件内的依赖解析死循环 by13
    }
...
var componentHooks = {
    ...
    $replace: false,
    $extend: null,
	$childrenHtml: "",  //保存子节点html字符串表示 by13
    ...
    for (var i = 0, obj; obj = componentQueue[i]; i++) {
        if (name === obj.fullName) {
            ...
            (function (host, hooks, elem, widget) {
				...
				var $id = elemOpts.$id || elemOpts.identifier || vmOpts.$id || vmOpts.identifier || generateID(widget)  //可通过config指定id by13
                host.configName = elemOpts.config || host.fullName  //设置configName供调用者操作 //by13
                ...
                //vmodel.$init(vmodel, elem)	//init操作移到后面 by13
                //global.$init(vmodel, elem)
                var nodes = elem.childNodes
					childrenHtml = elem.innerHTML
                //收集插入点
                var slots = {}, snode
				...
				//by13
				elem.msResolved = 1
                vmodel.$childrenHtml = childrenHtml;    //共$init操作
                vmodel.$init(vmodel, elem, host)    //添加host传递，便于reactive parameter实现 by13
                global.$init(vmodel, elem, host)
                vmodel.$childrenHtml = '';  //$init后马上清除
                ...
                var removeFn = avalon.bind(elem, "datasetchanged", function (e) {
                    ...
                    if (dependencies === 0) {
						...
						host.type = "widget"    //妈蛋你能别丢东西不 by13
    ...
    if (pre) {
        for (var i = 0, v; v = vmodels[i++]; ) {
            //if (v.hasOwnProperty(pre) && typeof v[pre] === "object") {
                ...
                //break
            //}
            // 扩展widgetOpt解析，可解析多级属性 by13
            if (isObjectInDeeper(v, pre)) {
                //var vmOptions = v[optName];
                var vmOptions = getPropertyValue(v, pre);
                return vmOptions.$model || vmOptions;
                break
            } else {
                if (v.hasOwnProperty(pre) && typeof v[pre] === "object") {
